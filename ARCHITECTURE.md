...
**Date:** 2025-05-25
**Context:** Reddit API allows 60 requests/minute for OAuth script apps. PRAW auto-handles rate limit headers but has a default `ratelimit_seconds=5` which aborts on longer waits.
...
2. Target 50 req/min (leaving 10 req/min headroom for PRAW's internal calls)
3. Add `PrawRateMonitor` that tracks request count and enforces pace — sleeps proportionally if rate exceeds target
**Why:** PRAW's default 5s limit causes crashes on long scraping runs. Raising to 300s makes the scraper resilient. Targeting 50/min instead of 60/min accounts for PRAW's internal pagination requests that you don't explicitly see. The monitor logs rate info every 10 requests so you can see pacing in action.
